...
def batchRgTensor(samples : np.ndarray, boxes : np.ndarray) -> np.ndarray:
    ...
    :param samples: np.ndarray, (...,n_chains, n_monomers,n_dim) e.g. (n_batch, n_frames, n_chains, n..., n_dim)
    :param boxes: np.ndarray, (...,n_dimensions), e.g. (n_batch, n_frames, n_dim)
    :return: np.ndarray ret.
    """
    if samples.ndim == 2:
        samples = np.expand_dims(samples, 0)
    if samples.ndim < boxes.ndim + 2:
        raise ValueError("NO~~~Are you using multiple box values for smaller dataset of samples?")
